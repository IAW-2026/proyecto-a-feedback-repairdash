  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <button
        ...
        className="w-full min-w-[180px] pl-10 pr-8 py-3 bg-[#271033] border border-brand-accent-soft rounded-lg text-[#fbdaf9] text-left cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-accent-strong transition-all duration-200"
      >
        {displayLabel}
      </button>
      <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-brand-accent-soft pointer-events-none" />
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-[#271033] border border-brand-accent-soft rounded-lg overflow-hidden shadow-lg">
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
