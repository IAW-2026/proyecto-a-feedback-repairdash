...
function ReporteCard({ reporte }: { reporte: ReporteCardData }) {
  return (
    <Link href={`/reportes/${reporte.id}`}>
      <div className="bg-brand-card rounded-lg p-[clamp(1rem,4vw,1.5rem)] border border-brand-accent-soft/30 hover:border-brand-accent-strong/40 hover:shadow-lg hover:shadow-brand-accent-strong/10 transition-all duration-300 hover:scale-[1.02] cursor-pointer group">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-[clamp(0.75rem,2vw,1rem)] mb-[clamp(0.75rem,2vw,1rem)]">
          <div className="min-w-0">
            <h3 className="font-gilroy font-bold text-brand-text-light break-words" style={{ fontSize: 'clamp(1rem, 3vw, 1.125rem)' }}>
              {reporte.nombreUsuario}
            </h3>
            <p className="text-brand-accent-mid" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.875rem)' }}>
        ...
        <div className="flex items-center gap-2 text-brand-accent-mid" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.875rem)' }}>
          <Calendar size={14} />
